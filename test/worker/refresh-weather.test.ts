 ...
 * tests. The "read site list" step reads camp:sites:v1 from test KV (seeded below with one site);
 * the fetch+digest step is mocked via the introspector (no live Open-Meteo, hard rule 4); the final
 * write step runs for real against test KV. One site → one chunk, so the step names are stable:
 * "read site list" → "fetch chunk 1/1" → "write wx:digest:v1".
 */
const CAMP_BLOB: CampsiteBlob = { fetchedAt: "2026-06-13T00:00:00Z", source: "osm", sites: [REYKJAVIK_ECO] };

// Tripwire: the fetch step is mocked, so the workflow must make zero live calls. If a step-name
// rename ever makes a mock miss, the real fetchDigests would hit Open-Meteo — throw loudly instead
// of reaching the network (hard rule 4: no live APIs in tests). The KV read step doesn't use fetch.
...
beforeEach(async () => {
  ...
  await env.KV.put(CAMP_SITES_KEY, JSON.stringify(CAMP_BLOB)); // the workflow's site list
});
afterEach(async () => {
