...
The pure scoring core (`scoreDay`, `findWindows`, policy config) plus the first REST endpoint, `GET/POST /api/windows`, returning the shared `Recommendation` shape. Weather comes from a checked-in fixture digest; the campsite list is one hardcoded site (Reykjavík Eco Campsite). No KV, no cron, no birds, `mapUrl` is a placeholder. This story is the TDD heart of the project: the canonical behavior table is implemented before anything else.
