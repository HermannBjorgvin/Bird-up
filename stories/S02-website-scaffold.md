...
- **Post-deploy review** (react-cloudflare-reviewer): approve with nits, 4 findings. Applied per owner: #2 `npm run deploy` script (check + build + deploy — bare `wrangler deploy` can ship a stale `dist/client`) and #4 nested-parens error message. Left per owner: #1 write hook blocks on warnings (`--max-warnings 0`) which conflicts with the 4-warning budget once a file legitimately spends it, and #3 plain `.js`/`.jsx` files only get `max-lines` (the `js.configs.recommended` baseline is scoped to ts/tsx) — both known, revisit if they bite a delegated agent.
