...
- Campsite markers colored by their score within the selected date range (continuous scale warm khaki → moss-green; **slate** where there's no qualifying window; tier in the popup). Sites are **clustered** (leaflet.markercluster) into per-area counts that split on zoom; each cluster bubble tints to the best score inside it (luminance-aware count text). Markers are divIcon **teardrop pins** (anchored at the tip): a filled, score-coloured pin where the site has a qualifying window, an **outline pin** where it has none (so "no window here" no longer reads as "bad weather here"). Popup: name, **lucide** facility icons, the quality line (`date-range · {tier} N/100`), a daily strip that stacks vertically (icon-labelled tMax/precip/gusts — no horizontal scrollbar), booking link (`bookingUrl`), "accepts Camping Card" badge. Base OSM tiles get a CSS aged-paper filter (vintage in light, wood-tinted in dark) — no tile-provider change.
...
A full-width horizontal bar that **always paints the whole forecast horizon** as a per-day heatmap of **royal-gold at a score-driven alpha** over the bar's theme-aware background (warm paper in light mode, dark wood in dark mode — so no bright patches on the dark bar): each day's intensity is its best score across all weather windows (`max` of the per-day `DailyScore.score`; the background shows through where no window covers it), faded smoothly between days at full height. The bar's heat reacts to the campsite filters: a window whose sites are all filtered out (offroad / beyond the drive cap) stops contributing, so days only good at filtered-out sites cool down on the bar too. Absolute scale, fully gold at score ≥ 40. The **date ticks are hidden on mobile** (they crowd a narrow bar); the range label still shows. **Date ticks** with labels run beneath the bar. Two day-snapped **brush handles** (thick, with a lucide `GripVertical` grab indicator) select a sub-range; the bar itself never shrinks. The selected range shows as the bar's header label. (The **minimum trip length** slider has moved out of the bar's header into the side-panel filters — see Controls.)

**Fetch model:** the site keeps **two** window sets. The **bar** fetches the whole forecast horizon and is re-queried only when the `minDays` slider changes. The **brush** sets a sub-range that re-queries `/api/windows` for that range (debounced): the server **clips** each window to the range and rescores it per-site over only those days ([02-scoring-policy.md](02-scoring-policy.md) "Per-request date range"), so the sidebar/map show windows that fit the brush with accurate per-site scores rather than windows spilling past it. When the brush spans the whole horizon there is nothing extra to fetch — the sidebar reuses the bar's full-horizon result. (This supersedes the earlier "fetch once, filter the brush in-browser" model: client-side filtering couldn't rescore each campsite over a sub-range, which only the server — holding each site's daily digest — can do.)
...
- **Filters** (top of the side panel, `web/src/components/Filters.tsx` + `lib/filters.ts`), in display order:
  ...
  The drive + family-car filters are **client-side** — they never re-query, narrowing the **map markers, the sidebar, and the timeline bar's heat together** (a window left with no passing site drops out of all three). Each of those two only renders when the loaded campsite data carries its attribute (`filterCapabilities`), so neither is a dead no-op before the enriched refresh lands; min trip length is always shown.
...
Kept deliberately minimal: the weather attribution (Open-Meteo, from the API `attribution[]`) and a link to the GitHub repo. OSM credits (campsite data + map tiles) live on the map's own Leaflet attribution control, so the footer filters them out rather than duplicating. The eBird verbatim string (when birds are on, [04-data-sources.md](04-data-sources.md)) still flows through `attribution[]`. The **MCP-endpoint hint and the policy-version line are omitted** until the MCP server ships.

## Theme

A design-system palette (warm earthy: gold / slate / dust / brown-red / khaki, plus a derived moss-green and wood-brown) drives two themes via `light-dark()` CSS tokens: **light = warm "field guide", dark = "old wood cabin"**. A header toggle (lucide Monitor / Sun / Moon) cycles **system → light → dark**, persisted in `localStorage` `tjaldur:theme:v1` (versioned) and applied before first paint by an inline script in `index.html` (no flash); `system` follows `prefers-color-scheme`. The score colors stay khaki→moss on the map; royal-gold is the selection/control accent (timeline heat, filter sliders).

## Local dev against the deployed API

`npm run dev:remote` runs the site with Vite HMR but points `/api/*` at the deployed worker (`VITE_API_BASE`) so styling can be previewed against real KV data without deploying. Unset in tests and production builds → same-origin relative paths.
